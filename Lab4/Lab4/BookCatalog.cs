...
public class Book: IComparable// Структура для хранения книг
{
    [JsonInclude]
    ...
    public Book(String Title, String Author, String Description, String ISBN, int KeyNum)
    {
        this.Title = Title;
        this.Author = Author;
        this.Description = Description;
    ...
    public Book() {}
    // изменяет книгу, value - поля через точка с запятой
    public bool UpdateBook(String value)
    {
        String[] Phrases = value.Split(';');
        if (Phrases.Length == 0 || Phrases.Length>4)
        {
            return false;
        }
        Title = Phrases[0];
...
public class BookCatalog
{
    ...
    public String GetBook(int ind)// книга по индексу
    {
        if (ind >= 1 && ind <= BookList.Count)
        {
            return BookList[ind - 1].Title + ";" + BookList[ind - 1].Author + ";" + BookList[ind - 1].Description;
        }
        else
        {
            return "";
    ...
    public bool UpdateBook(int ind, String value)
    {
        if (ind >= 1 && ind <= BookList.Count)
        {
            return BookList[ind - 1].UpdateBook(value);
        }
        else
        {
            return false;
    ...
    public bool DeleteBook(int ind)
    {
        if (ind >= 1 && ind <= BookList.Count)
        {
            BookList.RemoveAt(ind - 1);
            return true;
        }
        else
        {
    ...
    public bool SelectBooksByTitle(String Title, ref List<Book> Result)
    {
        Result.Clear();
        if (Title.Length == 0)
        {
        ...
        foreach (Book book in BookList) { 
            ...
            {
                Result.Add(new Book(book.Title, book.Author, book.Description, book.ISBN));
            }
        }
        return (Result.Count > 0);
    }
    // Найти книги по части названия
    public bool SelectBooksByPartOfTitle(String Title, ref List<Book> Result)
    {
        Result.Clear();
        if (Title.Length == 0)
        {
    ...
    public bool SelectBooksByAuthor(String Author, ref List<Book> Result)
    {
        Result.Clear();
        if (Author.Length == 0)
        {
    ...
    public bool SelectBooksByKeyWords(String StrKeyWords, ref List<Book> Result)
    {
        Result.Clear();
        ...
        foreach (Book book in BookList)
        {
            ...
            {
                Result.Add(new Book(book.Title, book.Author, book.Description, book.ISBN, KeyNum));
            }
        }
        Result.Sort();
        return (Result.Count > 0);
